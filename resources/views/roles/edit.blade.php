...
@section('title', "| Modifica Ruolo {$role->name}")

@section('content')
    <section id="roles-edit" class="pt-4">
        <div class="container">
                            ...
                            <div class="row mb-2">
                                <div class="col-12 col-md-6">
                                    <div class="form-floating mb-4">
                                        <input type="text" class="form-control input-violet shadow-sm @error('name') is-invalid @enderror" id="name" name="name" value="{{old('name', $role->name)}}" placeholder="Nome Ruolo" required>
                                ...
                                <div class="col-12">
                                    @foreach ($permissions as $key => $permission)
                                        <div class="form-check mb-3">
                                            <input {{$role->permissions->contains($permission) ? 'checked' : ''}} class="form-check-input" type="checkbox" id="permissions-{{$permission->id}}" name="permissions[{{$permission->id}}]"  value="{{$permission->id}}">
